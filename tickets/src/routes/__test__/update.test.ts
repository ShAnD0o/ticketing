...
it("401 if not authentication", async () => {
  const id = new mongoose.Types.ObjectId().toHexString();
  await request(app)
    .put(`/api/tickets/${id}`)
    .send({ title: "bla", price: 20 })
...
it("401 if user does not own a ticket ", async () => {
  ...
  await request(app)
    ...
    .set("Cookie", global.signup())
    .send({ title: "blaBlaBla", price: 2000 })
    .expect(401);
});
it("400 if invalod title or price", async () => {
